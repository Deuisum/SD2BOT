...
module.exports.unBlackList = async (message) => {
    ...
    const id = message.mentions.users.array()[0].id;
    await common.sql('DELETE FROM blacklist WHERE blockedUID = ?', [id])
        .catch((msg) => {
            message.channel.send(msg.err.toString())
        })
...
module.exports.purge = async (message, num) => {
    ...
    if (num > config.max_purge_num || num <= 0) {
        common.reply(message, `Error. ${num} is invalid. Please choose a number less than ${config.max_purge_num} and greater than 0.`);
        return;
    }
...
module.exports.isBlackListed = isBlackListed = async (id) => {
    const isBlackListed = await common.sql(`SELECT * FROM blacklist WHERE blockedUID = "${id}"`)
        .catch(() => { })
    if (isBlackListed.id.length === 0) {
        return false;
    }
    return true;
}
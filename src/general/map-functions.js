...
module.exports.pickUnbannedMap = pickUnbannedMap = (maps) => {
    const notBannedList = Object.keys(maps).filter(x => { return maps[x] });
...
module.exports.banning_unbanningMaps = (message, msgInput, unbanBool) => {
    ...
    const allMaps = { ...common.maps1v1, ...common.maps2v2, ...common.maps3v3, ...common.maps4v4 }
...
module.exports.bannedMaps = bannedMaps = (message) => {
    ...
    for (let i = 0; i < 13; i++) { //13 is max number of items in the 4 objects
        ...
        table.addRow(maps1[0], maps1[1], maps2[0], maps2[1], maps3[0], maps3[1], maps4[0], maps4[1]);
    }
    common.say(message, "``" + table.toString() + "``");
}
